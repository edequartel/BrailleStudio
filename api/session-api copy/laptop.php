...
<head>
  ...
  <style>
    .step-transition-screen {
      ...
      padding: clamp(4rem, 12vh, 7rem) 1.5rem 1.5rem;
      background: rgba(15, 23, 42, 0.34);
      backdrop-filter: blur(2px);
    }

    .step-transition-screen[hidden] {
    ...
    .step-transition-panel {
      ...
      animation: step-transition-pop .16s ease-out;
    }

    @keyframes step-transition-pop {
      from {
        opacity: 0;
...
<body class="bg-body">
        ...
        <div class="page-body">
          <div id="sessionStageRow" class="row g-3 justify-content-center">
            ...
            <div id="playerCard" class="col-12 col-lg-8 d-none" hidden>
              <div id="connectionReadyAlert" class="alert alert-success d-none" role="status" hidden>
                <div class="d-flex align-items-center gap-3">
                  <span class="avatar avatar-md bg-success-lt text-success">
                    <i class="ti ti-circle-check" aria-hidden="true"></i>
                  </span>
                  <div>
                    <div class="fw-bold">Telefoon verbonden</div>
                    <div>De gebruiker is klaar om een step-link code te scannen.</div>
                  </div>
                </div>
              </div>
              <div id="stepTransitionScreen" class="step-transition-screen d-none" role="status" aria-live="polite" hidden>
                <div class="step-transition-panel text-center px-4 py-5">
                  <div id="stepTransitionSpinner" class="spinner-border text-primary mb-4" role="presentation"></div>
                  <div id="stepTransitionPretitle" class="text-secondary small text-uppercase fw-bold mb-2">Step-link ontvangen</div>
                  <h2 id="stepTransitionTitle" class="h1 mb-2">Stap wordt gestart</h2>
                  <div id="stepTransitionText" class="text-secondary">De les wordt geladen.</div>
                </div>
              </div>
  ...
  <script>
    ...
    function renderConnectionReadyAlert(visible = false, message = '') {
      renderPlayerStatusAlert({
        visible,
        type: 'success',
        icon: 'ti ti-circle-check',
        title: 'Telefoon verbonden',
        message: message || 'De gebruiker is klaar om een step-link code te scannen.'
      });
    }

    function renderPlayerStatusAlert({
      visible = true,
      title = 'Telefoon verbonden',
      message = 'De gebruiker is klaar om een step-link code te scannen.',
      type = 'success',
      icon = 'ti ti-circle-check'
    } = {}) {
      const alert = $('connectionReadyAlert');
      if (!alert) return;
      const variants = {
      ...
      alert.className = `${variants[type] || variants.success}${visible ? '' : ' d-none'}`;
      alert.hidden = !visible;
      const avatar = alert.querySelector('.avatar');
      if (avatar) {
        avatar.className = `avatar avatar-md ${
          type === 'info' ? 'bg-info-lt text-info' :
          type === 'warning' ? 'bg-warning-lt text-warning' :
          type === 'danger' ? 'bg-danger-lt text-danger' :
          'bg-success-lt text-success'
        }`;
      }
      const iconNode = alert.querySelector('.avatar i');
      if (iconNode) {
        iconNode.className = icon;
      }
      const titleNode = alert.querySelector('.fw-bold');
      if (titleNode) {
        titleNode.textContent = title;
      }
      const messageNode = alert.querySelector('.fw-bold + div');
      if (messageNode) {
        messageNode.textContent = message;
      }
    }

    function renderStepTransitionScreen({
      visible = false,
      pretitle = 'Step-link ontvangen',
      title = 'Stap wordt gestart',
      message = 'De les wordt geladen.',
      busy = true
    } = {}) {
      const screen = $('stepTransitionScreen');
      if (!screen) return;
      screen.hidden = !visible;
      screen.classList.toggle('d-none', !visible);
      const spinner = $('stepTransitionSpinner');
      if (spinner) {
        spinner.hidden = !busy;
        spinner.classList.toggle('d-none', !busy);
      }
      const pretitleNode = $('stepTransitionPretitle');
      if (pretitleNode) {
        pretitleNode.textContent = pretitle;
      }
      const titleNode = $('stepTransitionTitle');
      if (titleNode) {
        titleNode.textContent = title;
      }
      const messageNode = $('stepTransitionText');
      if (messageNode) {
        messageNode.textContent = message;
      }
    }

    function renderUserActionPopup(title, message) {
      renderStepTransitionScreen({
        visible: true,
        pretitle: 'Actie nodig',
        title,
        message,
        busy: false
      });
    ...
    async function applyResolvedToBlockly(payload, { autoOpen = true } = {}) {
      ...
      customizeBlocklyFrameUi();
      syncBlocklyFrameHeight();
      const applyResult = await app.applyResolvedSessionPayload(payload, { autoRun: false, force: true });
      const runPromise = typeof app.runCurrentWorkspace === 'function'
        ? app.runCurrentWorkspace()
        : app.applyResolvedSessionPayload(payload, { autoRun: true, force: true });
      window.setTimeout(() => {
        renderStepTransitionScreen({ visible: false });
      }, 500);
      Promise.resolve(runPromise).catch((err) => {
        logLine('Background Blockly run failed.', {
          code: payload?.code || '',
    ...
    function renderSession(session = null) {
      ...
      if (badge) {
        badge.className = joined
          ? 'badge bg-success-lt text-success'
          : (active ? 'badge bg-primary-lt text-primary' : 'badge bg-secondary-lt');
        const dot = badge.querySelector('.status-dot');
        if (dot) {
          dot.className = `status-dot status-dot-animated ${joined ? 'bg-success' : (active ? 'bg-primary' : 'bg-secondary')} me-2`;
        }
        const label = badge.querySelector('span:last-child');
        if (label) {
          label.textContent = joined
            ? 'Klaar voor step-link'
            : (active ? `Sessie gestart: ${session.sessionId}` : 'Nog geen actieve les');
        }
      }
    ...
    function resetSessionRuntimeState() {
      ...
      renderConnectionReadyAlert(false);
      renderStepTransitionScreen({ visible: false });
    ...
    async function startSession() {
      ...
      setResolveStatus('Sessie gestart. Laat de gebruiker de QR-code scannen.', 'success');
      renderUserActionPopup(
        'Scan de sessie QR-code',
        'Laat de gebruiker de QR-code op deze laptop scannen om de telefoon te verbinden.'
      );
      startSessionPolling();
    ...
    function handleSessionJoined(status) {
      ...
      openBlockly();
      renderConnectionReadyAlert(true);
      renderUserActionPopup(
        'Scan de step-link code',
        'De telefoon is verbonden. Laat de gebruiker nu een step-link code scannen.'
      );
    ...
    function startSessionJoinPolling() {
      ...
      sessionJoinPollTimer = window.setInterval(poll, 350);
      poll();
    }

    async function waitForSessionStatus(sessionId, since = '', joinedAt = '') {
      const requestStartedAt = Date.now();
      const status = await postJson(WAIT_URL, { sessionId, since, joinedAt });
    ...
    async function releaseActiveStepAfterStop() {
      ...
      renderUserActionPopup(
        'Scan een nieuwe step-link',
        'De stap is gestopt. Laat de gebruiker een nieuwe step-link code scannen.'
      );
      renderConnectionReadyAlert(true, 'Scan een nieuwe step-link code om verder te gaan.');
    ...
    async function monitorActiveStepCompletion(stepKey, payload) {
      ...
      while (watchToken === activeStepCompletionWatchToken && lastStartedStepKey === stepKey && lastStartedStepStillActive) {
        ...
        renderUserActionPopup(
          'Scan een nieuwe step-link',
          'De stap is afgerond. Laat de gebruiker een nieuwe step-link code scannen.'
        );
        renderConnectionReadyAlert(true, 'Scan een nieuwe step-link code om verder te gaan.');
    ...
    async function handleStartedStep(status) {
      ...
      renderPlayerStatusAlert({
        ...
        type: 'info',
        icon: 'ti ti-loader-2',
        title: 'Stap wordt geladen',
        message: `Step-link ${payload.code || payload.stepId || ''} is ontvangen. De les wordt gestart.`
      });
      renderStepTransitionScreen({
        visible: true,
        pretitle: 'Step-link ontvangen',
        title: 'Stap wordt gestart',
        message: `Step-link ${payload.code || payload.stepId || ''} is ontvangen. Het script wordt geladen.`,
        busy: true
      });
      setResolveStatus(`Step-link ${payload.code || ''} ontvangen. De les wordt geladen...`, 'info');
      $('sessionStateText').textContent = 'Stap laden';
      try {
      ...
      setResolveStatus(`Stap ${payload.code || ''} wordt gestart...`, 'info');
      renderPlayerStatusAlert({ visible: false });
      renderStepTransitionScreen({
        visible: true,
        pretitle: 'Step-link ontvangen',
        title: 'Script wordt gestart',
        message: `Stap ${payload.code || payload.stepId || ''} wordt nu in Blockly geladen.`,
        busy: true
      });
      $('sessionStateText').textContent = 'Stap actief';
      ...
      activeStepRunPromise = (async () => {
        try {
          ...
          if (startResult?.runtimeStillActive) {
            ...
            renderUserActionPopup(
              'Scan een nieuwe step-link',
              'De stap is direct afgerond. Laat de gebruiker een nieuwe step-link code scannen.'
            );
          }
          renderConnectionReadyAlert(!startResult?.runtimeStillActive, 'Scan een nieuwe step-link code om verder te gaan.');
          ...
          renderUserActionPopup(
            'Scan opnieuw',
            'De telefoon blijft verbonden. Laat de gebruiker opnieuw scannen of kies een andere step-link.'
          );
          renderConnectionReadyAlert(true, 'De telefoon blijft verbonden. Scan opnieuw of probeer een andere step-link.');
    ...
    function startSessionPolling() {
      ...
      const loop = async () => {
        ...
        let joinedAt = String(getActiveSession()?.joinedAt || '').trim();
        if (!since) {
          try {
            ...
            joinedAt = String(initial?.joinedAt || joinedAt || '').trim();
        ...
        while (loopToken === sessionWaitLoopToken) {
          try {
            const status = await waitForSessionStatus(session.sessionId, since, joinedAt);
            if (loopToken !== sessionWaitLoopToken) return;
            if (handleSessionJoined(status)) {
              joinedAt = String(status?.joinedAt || joinedAt || '').trim();
            }
            if (status?.resolvedChanged && status?.lastResolvedAt) {
