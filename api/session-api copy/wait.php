...
$joinedSince = trim((string)($input['joinedAt'] ?? ''));
...
while (true) {
    ...
    $resolvedChanged = $lastResolvedAt !== '' && $lastResolvedAt !== $since;
    $joinedChanged = $joinedAt !== '' && $joinedAt !== $joinedSince;
    $changed = $resolvedChanged || $joinedChanged;
    ...
    if ($changed || microtime(true) >= $timeoutAt) {
        session_api_respond([
            ...
            'changed' => $changed,
            'joinedChanged' => $joinedChanged,
            'resolvedChanged' => $resolvedChanged,
            'timedOut' => !$changed,
